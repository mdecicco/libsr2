...
namespace sr2 {
    u32 srfeBasicLayout1::ActiveCount = 0;
    u32 srfeBasicLayout1::InitializedCount = 0;
    srfeBasicLayout1::Widgets* srfeBasicLayout1::WidgetsInstance = nullptr;
    srfeBasicLayout1::srfeBasicLayout1(const char* name, const WidgetRef<ui2Master>& master) : srfeBackground(name, master) {
    ...
    srfeBasicLayout1::~srfeBasicLayout1() {
        deinitScreen();
        if (m_isActive) ActiveCount--;
    }

    void srfeBasicLayout1::setActive(bool p1) {
        bool wasActive = m_isActive;
        srfeBackground::setActive(p1);
    ...
    void srfeBasicLayout1::initScreen() {
        srfeBackground::initScreen();
        InitializedCount++;

        if (!WidgetsInstance) initWidgets();

        WidgetsInstance->show_ui_timer->addListener(this, WidgetEventType::TimerFinished, (WidgetEventCallback)&srfeBasicLayout1::interceptEvent);
    }

    void srfeBasicLayout1::deinitScreen() {
    ...
    void srfeBasicLayout1::method_0x108() {
    ...
    void srfeBasicLayout1::method_0x110() {
    ...
    bool srfeBasicLayout1::method_0x130() {
        return true;
    }
    
    bool srfeBasicLayout1::method_0x138() {
        return true;
    }

    void srfeBasicLayout1::interceptEvent(const ui::NamedRef& source, WidgetEventType event, const WidgetRef<ui2EventData>& data) {
        if (!m_isActive) return;
        method_0x110();
    }

    void srfeBasicLayout1::initWidgets() {
